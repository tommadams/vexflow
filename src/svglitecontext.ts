...
class Node {
  ...
  toString(indent: string = ''): string {
    ...
    if (!this.children) {
      result += `>${this.textContent}</${this.type}>\n`;
    } else {
      result += `>${this.textContent}\n`;
      const childIndent = indent + '  ';
      for (const child of this.children) {
        result += child.toString(childIndent);
      }
...
class TextMeasurer {
  ...
  measureText(text: string, family: string, size: number, weight: string): {width: number, height: number} {
...
export class SVGLiteContext implements RenderContext {
  element: HTMLElement; // the parent DOM object
  svg: Node = new Node('svg');
  width: number = 0;
  height: number = 0;
  path: string = '';
  pen: { x: number; y: number } = { x: NaN, y: NaN };
  ...
  constructor(element: HTMLElement) {
    ...
    window.requestAnimationFrame(() => {
      this.element.innerHTML = this.svg.toString();
    });
  }

  // Allow grouping elements in containers for interactivity.
  openGroup(cls: string, id?: string, attrs?: { pointerBBox: boolean }): Node {
    const group = new Node('g');
  ...
  applyAttributes(element: Node, attributes: Attributes): Node {
    const attrNamesToIgnore = attrNamesToIgnoreMap[element.type];
  ...
  measureText(text: string): { width: number, height: number} {
    const size = Number(this.attributes['font-size'].match(/\d+/));
    return textMeasurer.measureText(
      text, this.attributes['font-family'], size, this.attributes['font-weight']);
  }
